...
function findRandomEmptyCell(board) {
  let i = Math.floor(
    Math.random() * (CONSTANTS.boardItems.boardLength - 1) + 1
  );
  let j = Math.floor(
    Math.random() * (CONSTANTS.boardItems.boardLength - 1) + 1
  );
  while (
    !(board.getObjectInLocation({ x: i, y: j }) == CONSTANTS.boardItems.empty)
  ) {
...
function initializeBoard(numOfGhosts, numOfFoods) {
  ...
  for (let i = 0; i < numOfGhosts && i < 4; i++) {
    board.ghosts.push(
      new Ghost(CONSTANTS.corners[i], CONSTANTS.colorPalette.ghostColor)
    );
    ghostNotMove.push(false);
  }
  CONSTANTS.obstacles.forEach((coordinate) =>
    board.obstacles.push(
      new Obstacle(coordinate, CONSTANTS.colorPalette.obstacleColor)
    )
  );
  ...
  for (let i = 0; i < numOfFoods; i++) {
    let currentFood = new Food(
      findRandomEmptyCell(board),
      CONSTANTS.colorPalette.foodColor
    );
...
function initializeGame() {
  ...
  changes.onchange = () => {
    ...
    if (!(numOfFoods >= 0 && numOfFoods <= 70)) {
      ...
      numOfFoods = 30;
      labelFoods.value = 30;
    } else if (!(numOfGhosts > 0 && numOfGhosts <= )) {
