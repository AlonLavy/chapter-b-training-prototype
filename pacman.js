...
export class Pacman extends BoardItem {
  ...
  draw() {
    ...
    context.arc(
      ...
      this.center.y,
      30,
      0.15 * Math.PI + rotation,
      1.85 * Math.PI + rotation
    ); // half circle
    context.lineTo(this.center.x, this.center.y);
    context.fillStyle = CONSTANTS.colorPalette.pacmanColor;
    context.fill();
    context.beginPath();
    switch (rotation) {
    ...
    context.fillStyle = CONSTANTS.colorPalette.eyeColor;
    context.fill();
  }
  ...
  #isOnFood(board) {
    if (
      board.getObjectInLocation(this.location) === CONSTANTS.boardItems.food
    ) {
